# Marine Bioacoustic Identifier: _Stenella longirostris_
...
<p align=center>
<img src='./images/spinners_ks.jpeg' alt='spinner dolphins, photo by Kawika Singson'>
<br>Spinner dolphins (_S. longirostris_) along the Hawai'i coast <br>
</p> 
...
<br>Spectrogram containing humpback song and anthropogenic input <br>
</p> 
...
Once the superior model was identified through validation, this model was refit 4 times (a total of 5 fits) and average performance was analyzed.  See [notebook_06](https://github.com/welcometohelen/marine_bioacoustic_identifier/blob/main/notebooks/06_holdout_validation.ipynb) for methods and results.
...
For 70% of the model versions tested here, exposing the model to augmented data during training yielded improved performance across all classification metrics.  We can not definitively say if this is due to the _masks_ alone, or due to the increased p:n training ratio resulting from the addition of masked data.

---

### Conclusions

vX likely represents the near-maximum recall achievable with this depth of neural network. External server options were pursued but still did not allow for fitting the entire dataset at once, let alone increasing CNN depth or complexity.

So far, this project is hopefully a sound basis for pursuing more advanced and effective models for MM acoustic identification using CNN. Next steps with this existing structure include:
...
4. Experiment with different methods of scaling the spectrograms. There are several methods used in relevant literature, including no scaling at all.


The main obstacle to a improving a model with these data is increased memory and GPUs. External servers were pursued during this work, but GPUs were unavailable through multiple providers; and a c5ad.4xlarge instance with AWS could not fit all data to the current shallow network without crashing from OOM.  With increased computing resources, priority steps include:
...
<p align=center>
<img src='./images/spinners2_h.jpg'>
</p>  
