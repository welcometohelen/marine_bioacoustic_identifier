...
<br>vX_masked best performance in a single fit. Deteriorates when averaging scores over multiple fits. <br>
</p>  
...
1. Quadruple the number of filters in each conv2d layer of the superior model. How much mileage can this architecture get just through increasing filter number?
...
3. Deepen the NN: related literature points to VGG-19, ResNet-50, and AlexNet as deep-layer NNs that have been employed for marine mammal acoustic tasks with better success
    * There are supposedly keras supported versions of [VGG-19](https://keras.io/api/applications/vgg/) and [ResNet-50](https://keras.io/api/applications/resnet/), but they will require significant tuning and/or gridsearching to tailor to spectrograms. Though not a pre-existing keras architecture, there are also examples of [how to code AlexNet for tensorflow](https://analyticsindiamag.com/hands-on-guide-to-implementing-alexnet-with-keras-for-multi-class-image-classification/).
